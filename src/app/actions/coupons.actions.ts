...
export async function getCoupons() {
  try {
    ...
    const res = await fetch(`${SUPABASE_URL}/rest/v1/coupons?code=neq.2X1_AUTO&order=created_at.desc`, {
      headers,
      cache: 'no-store',
    });
...
export async function updateCoupon(id: string, formData: Partial<CouponFormData>) {
  try {
    ...
    if (formData.zone_id !== undefined) body.zone_id = formData.zone_id || null;
